...
Next we can check estimate the expected detection delay for the above example:

```{r expected detection delay}
est_detdel(NJ, NK, rho$both["jump"], rho$both["kink"], k, 1, 0.01)
```

Last we can estimate the probability for each detector to detect the change first in the above example:

```{r expected detection type probability}
