...
t_tuning_mult <- function(NJ, NK, k,tau = NULL, eta = NULL, arl = NULL, r = 1000,
                     data = NULL){
  ...
  if(is.null(data)){
    #generates r sets of data and calculates the maximal test statistics
    max_Tstat <- replicate(r, max_Tstat_calc_mult(NJ, NK, tau, k), simplify = FALSE)
  }
  else{
    r <- ncol(data)
    max_Tstat <- sapply(1:r,function(i) max_Tstat_calc_mult(NJ, NK, tau,
                                                       k, data[,i]), simplify = FALSE)
  }
  ...
  if(length(NJ) == 0){
    ...
    rhoj <- rho[1:length(NJ)]
    rhok <- rho[(length(NJ)+1):(length(NJ) + length(NK))]
  }

  thresholds = list(jump =  unname(rho[1:length(NJ)]), kink = unname(rhok))
...
max_Tstat_calc_mult <- function(NJ, NK, tau, k, datavec = NULL){
  ...
  if(jump){
    SJ <- sapply(NJ,S_init, data = data[1:(3 * max(NJ, NK)) ])
  }
  if(kink){
    SK <- sapply(NK,S_init, data = data[1:(3 * max(NJ, NK)) ])
    W <- sapply(NK,W_init, data = data[1:(3 * max(NJ, NK)) ])
  }
  ...
  for(i in 1:tau){
    ...
    if(jump){
      ...
      #number of observations in last bin
      rJ <- sapply(NJ, function(x) (i -1) %% x + 1)
      ...
      for(j in 1:length(NJ)){
        ...
        SJ[3,j] <- SJ[3,j] + data[i + 3 * NJ[j]]
      }

      #calculate new test statistics
      Tstat_jump <- rbind(Tstat_jump,
                          sapply(1:length(NJ),
                                 function(i) Tstat_j_calc(S = SJ[,i], N = NJ[i],
                                                          r = rJ[i])))

    }
    if(kink){
      rK <- sapply(NK, function(x) (i -1) %% x + 1)
      ...
      for(j in 1:length(NK)){
        ...
        SK[3,j] <- SK[3,j] + data[i + 3 * NK[j]]
        W[3,j] <- W[3,j] + rK[j] * data[i + 3 * NK[j]]
      }

      Tstat_kink <- rbind(Tstat_kink,
                          sapply(1:length(NK),
  ...
  #take maximal test statistic
  if(jump){
    Tstat_jump <-apply(abs(Tstat_jump), 2, max)
  }
  if(kink){
    Tstat_kink <- apply(abs(Tstat_kink),2,max)
  }
